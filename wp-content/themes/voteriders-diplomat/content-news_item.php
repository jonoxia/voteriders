			...
			<?php if ($post_pod_type !== 'quote') { ?>
				...
				<div class="entry-content">
					...
					if( strpos( $post->post_content, '<!--more-->' ) ){
						the_content();
					} else if(get_post_meta($post->ID, 'wpcf-excerpt', true)){
...
//Might need to nest this deeper
						...
						$exContent = get_post_meta($post->ID, 'wpcf-excerpt', true);
						$txt = do_shortcode($exContent);
						$txt = strip_tags($txt);
						$txt = mb_substr($txt, 0, 250) . " ...";
						echo '<p>'.$txt.'</p>'.'<a href="'. $articleLink .'"> Read the full article...</a>';
