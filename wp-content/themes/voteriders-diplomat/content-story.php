...
*/
					...
					if( strpos( $post->post_content, '<!--more-->' ) ){
						the_content();
					} else if(get_post_meta($post->ID, 'wpcf-storyvideodescription', true)){
						
//Might need to nest this deeper


						$refURL = types_render_field("url");

						$articleLink = ($refURL) ? $refURL : get_permalink();						
						$exContent = get_post_meta($post->ID, 'wpcf-storyvideodescription', true);
						$txt = do_shortcode($exContent);
						$txt = strip_tags($txt);
						$txt = mb_substr($txt, 0, 250) . " ...";
						echo '<p>'.$txt.'</p>'.'<a href="'. $articleLink .'"> Read more...</a>';
						...
						if (TMM::get_option("excerpt_symbols_count") !== '0') {
							...
							if (empty($post->post_excerpt)) {
								...
								$txt = strip_tags($txt);
								$txt = mb_substr($txt, 0, $symbols_count) . " ... EP";
							} else {
								$txt = mb_substr($post->post_excerpt, 0, $symbols_count) . " ...";
							}
							echo '<p>'.$txt.'</p>'.'<a href="'. get_permalink().'"> Read the full article...</a>';
						}
					}
					?>
				</div>
				...
				<?php  if ($show_post_metadata !== '0') { ?>
					...
					<div class="right">
						<?php  if ($blog_listing_show_date !== '0') { ?>
							...
							<span class="posted-on"><?php echo get_the_date($date_format); ?></a></span>
